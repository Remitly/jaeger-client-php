    ...
    private function makeZipkinBatch(array $spans): array
    {
        ...
        foreach ($spans as $span) {
            ...
            if (strlen($traceId) > 16) {
                $traceIdHigh = self::hexToInt64(substr($traceId, -32, -16));
            } else {
                $traceIdHigh = null;
            }

            $traceId = self::hexToInt64(substr($traceId, -16));
            $spanId = self::hexToInt64($span->getContext()->getSpanId());
            $parentId = $span->getContext()->getParentId();
            if ($parentId !== null) {
                $parentId = self::hexToInt64($parentId);
            }
